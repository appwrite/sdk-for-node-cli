...
const functionsRetryBuild = async ({ buildId, parseOutput = true, sdk = undefined}) => {
...
const functionsUpdateDeployment = async ({ functionId, deployment, parseOutput = true, sdk = undefined}) => {
    /* @param {string} functionId */
    /* @param {string} deployment */

    let client = !sdk ? await sdkForProject() : sdk;
    let path = '/functions/{functionId}/deployment'.replace('{functionId}', functionId);
    let payload = {};
...
const functionsCreateDeployment = async ({ functionId, entrypoint, code, deploy, parseOutput = true, sdk = undefined, onProgress = () => {}}) => {
    /* @param {string} functionId */
    ...
    if (typeof deploy !== 'undefined') {
        payload['deploy'] = deploy.toString();
    }
...
functions
    ...
    .requiredOption(`--functionId <functionId>`, `Function ID.`)
    .requiredOption(`--deployment <deployment>`, `Deployment ID.`)
...
functions
    ...
    .requiredOption(`--deploy <deploy>`, `Automatically deploy the function when it is finished building.`, parseBool)
