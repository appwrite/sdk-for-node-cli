...
const functionsListBuilds = async ({ limit, offset, search, cursor, cursorDirection, parseOutput = true, sdk = undefined, json }) => {
    /* @param {number} limit */
    /* @param {number} offset */
    /* @param {string} search */
    /* @param {string} cursor */
...
const functionsGetBuild = async ({ buildId, parseOutput = true, sdk = undefined, json }) => {
    /* @param {string} buildId */

    let client = !sdk ? await sdkForProject() : sdk;
    let path = '/builds/{buildId}'.replace('{buildId}', buildId);
    let payload = {};
    const response = await client.call('get', path, {
        'content-type': 'application/json',
    }, payload);
...
const functionsRetryBuild = async ({ buildId, parseOutput = true, sdk = undefined, json }) => {
    /* @param {string} buildId */
...
const functionsCreateExecution = async ({ functionId, data, async, parseOutput = true, sdk = undefined, json }) => {
    /* @param {string} functionId */
    /* @param {string} data */
    /* @param {boolean} async */
...
const functionsCreateTag = async ({ functionId, entrypoint, code, automaticDeploy, parseOutput = true, sdk = undefined, json }) => {
    /* @param {string} functionId */
    ...
    childProcess.execSync(`tar --cd '${_path.join(_path.dirname(folderPath), folderName)}' -zcvf code.tar.gz .`, { stdio: 'pipe' });
    ...
    if (typeof automaticDeploy !== 'undefined') {
        payload['automaticDeploy'] = automaticDeploy.toString();
    }
...
functions
    .command(`listBuilds`)
    .description(`Get a list of all the current user build logs. You can use the query params to filter your results. On admin mode, this endpoint will return a list of all of the project's executions. [Learn more about different API modes](/docs/admin).`)
    ...
    .option(`--cursor <cursor>`, `ID of the build used as the starting point for the query, excluding the build itself. Should be used for efficient pagination when working with large sets of data.`)
    .option(`--cursorDirection <cursorDirection>`, `Direction of the cursor.`)
    .option(`--json`, `Output in JSON format`)
    .action(actionRunner(functionsListBuilds))
...
functions
    ...
    .option(`--json`, `Output in JSON format`)
    .action(actionRunner(functionsGetBuild))
...
functions
    ...
    .requiredOption(`--functionId <functionId>`, `Function ID.`)
    .option(`--search <search>`, `Search term to filter your list results. Max length: 256 chars.`)
    .option(`--limit <limit>`, `Maximum number of tags to return in response. By default will return maximum 25 results. Maximum of 100 results allowed per request.`, parseInteger)
    .option(`--offset <offset>`, `Offset value. The default value is 0. Use this value to manage pagination. [learn more about pagination](https://appwrite.io/docs/pagination)`, parseInteger)
...
functions
    ...
    .requiredOption(`--automaticDeploy <automaticDeploy>`, `Automatically deploy the function when it is finished building.`, parseBool)
    .option(`--json`, `Output in JSON format`)
