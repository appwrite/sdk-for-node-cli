...
const storageListFiles = async ({ search, limit, offset, cursor, cursorDirection, orderType, parseOutput = true, sdk = undefined}) => {
    /* @param {string} search */
    /* @param {number} limit */
    /* @param {number} offset */
    /* @param {string} cursor */
...
const storageCreateFile = async ({ fileId, file, read, write, parseOutput = true, sdk = undefined}) => {
    /* @param {string} fileId */
    /* @param {File} file */
    /* @param {string[]} read */
    /* @param {string[]} write */
...
const storageGetFile = async ({ fileId, parseOutput = true, sdk = undefined}) => {
    /* @param {string} fileId */
...
const storageUpdateFile = async ({ fileId, read, write, parseOutput = true, sdk = undefined}) => {
    /* @param {string} fileId */
    /* @param {string[]} read */
    /* @param {string[]} write */
...
const storageDeleteFile = async ({ fileId, parseOutput = true, sdk = undefined}) => {
    /* @param {string} fileId */
...
const storageGetFileDownload = async ({ fileId, parseOutput = true, sdk = undefined}) => {
    /* @param {string} fileId */
...
const storageGetFilePreview = async ({ fileId, width, height, gravity, quality, borderWidth, borderColor, borderRadius, opacity, rotation, background, output, parseOutput = true, sdk = undefined}) => {
    /* @param {string} fileId */
    /* @param {number} width */
    /* @param {number} height */
...
const storageGetFileView = async ({ fileId, parseOutput = true, sdk = undefined}) => {
    /* @param {string} fileId */
...
storage
    ...
    .option("--limit <limit>", "Maximum number of files to return in response. By default will return maximum 25 results. Maximum of 100 results allowed per request.")
...
storage
    ...
    .option("--quality <quality>", "Preview image quality. Pass an integer between 0 to 100. Defaults to 100.")
    .option("--borderWidth <borderWidth>", "Preview image border in pixels. Pass an integer between 0 to 100. Defaults to 0.")
    .option("--borderColor <borderColor>", "Preview image border color. Use a valid HEX color, no # is needed for prefix.")
    .option("--borderRadius <borderRadius>", "Preview image border radius in pixels. Pass an integer between 0 to 4000.")
    .option("--opacity <opacity>", "Preview image opacity. Only works with images having an alpha channel (like png). Pass a number between 0 to 1.")
    .option("--rotation <rotation>", "Preview image rotation in degrees. Pass an integer between -360 and 360.")
